...
function parseCsvFile(file) {
  const templateHeadersLower = new Map(
    TEMPLATE_HEADERS.map((h) => [h.toLowerCase(), h])
  );
  ...
  return new Promise((resolve, reject) => {
    Papa.parse(file, {
      ...
      transformHeader: (header) => {
        ...
        if (templateHeadersLower.has(lower)) {
          return templateHeadersLower.get(lower);
        }
        if (lower === "country") return "countryCode";
        if (lower === "country code") return "countryCode";
        if (lower === "store name") return "name";
        return trimmed;
      },
...
export default function BulkStoreImportDialog({ open, onOpenChange, stores, categories, countries, onImported }) {
  ...
  async function selectFile(file) {
    ...
    try {
      ...
      void runDryValidation(file, content);
    } catch {
  ...
  async function runDryValidation(fileToValidate = null, contentToValidate = "") {
    const file = fileToValidate || selectedFile;
    const content = contentToValidate || selectedFileContent;
    if (!file) {
  ...
  async function handleImport() {
    ...
    if (!rowsToImport.length) {
      ...
      rowsToImport = await runDryValidation();
    }

    if (!rowsToImport.length) {
      toast.error("No valid store rows to import.");
      return;
    }
